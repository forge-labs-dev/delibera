...
class ScoreWeights:
    ...
    Defaults are tuned for v1:
    - Reward evidence coverage (supported fact claims)
    - Reward having evidence (up to diminishing returns)
    - Penalize weak claims (inference without full support)
    - Heavily penalize unsupported claims
    - Very heavily penalize blocking objections
    ...
    evidence_count: float = 0.2
    weak_claims: float = -0.3
    unsupported_claims: float = -1.0
    blocking_objections: float = -2.0
    nonblocking_objections: float = -0.3
